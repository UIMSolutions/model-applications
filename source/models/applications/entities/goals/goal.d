...
static this() {
  ...
  createEntities["aplGoal"] = (Json json) => APLGoal(json); 
  createEntities["goal"] = (Json json) => APLGoal(json); 
}

// Target objective for a user or a team for a specified time period.
  class DAPLGoal : DOOPEntity {
  this() { super();
    this.addValues([
      ...
      "stateCode": StateCodeAttribute, // Shows whether the goal is open, completed, or canceled. Completed and canceled goals are read-only and can't be edited."]),
      "stateCode_display": StringAttribute, // 
      "statusCode": StatusCodeAttribute, //Select the goal's status."]),
  ...
  override string entityClass() { return "aplGoal"; }
  override string entityClasses() { return "aplGoals"; }
...
auto APLGoal() { return new DAPLGoal; } 
auto APLGoal(Json json) { return new DAPLGoal(json); } 
